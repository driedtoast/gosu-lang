...
public class ResourcePath {
  ...
  public static ResourcePath parse(String pathString) {
    ...
    if (pathString.startsWith(WINDOWS_NETWORK_ROOT)) {
      ...
      // TODO - AHK - Verify it's just a-zA-Z and not just any-old alpha-numeric character
    ...
    List<String> results = tokenizePathFragment(pathString, lastIndex);

    results = normalizePath(results);
    // TODO - AHK - Normalize out . and .. if desired, as well as any \ or / elements

    return construct(rootElement, results, results.size() - 1);
  }

  private static List<String> tokenizePathFragment(String pathString, int lastIndex) {
  ...
  private static List<String> normalizePath(List<String> pathElements) {
    List<String> results = new ArrayList<String>();
  ...
  public ResourcePath join(String otherPath) {
    ...
    List<String> components = tokenizePathFragment(otherPath, 0);
  ...
  // TODO - AHK - How do we determine if the path is case-sensitive or not?
  /**
