...
/**
 * CaseInsensitiveHashMap provides a Map implementation that treats keys case-insensitively.  While it's possible
 * to implement a case-insensitive map by wrapping a normal HashMap object, doing so in a highly-performant way
 * turns out to be difficult.  Calling toLowerCase() on all keys in the table isn't viable because it loses information
 * about the proper case of the key and has the performance hit of allocating an entirely new String every time.
 * Wrapping all keys implicitly in a CaseInsensitiveCharSequence also requires additional object allocation and
 * imposes overhead on lookup operations.  As a result, we have our own implementation.
 *
 * This implementation uses the same basic data structure as the standard Java HashMap implementation (an array of
 * buckets whose entries form a linked-list) and as a result should have the same performance characteristics.
 * It also has the same API in terms of exposed constructors for the sake of easy compatibility between the two classes.
 *
