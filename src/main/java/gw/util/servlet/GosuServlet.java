...
public class GosuServlet extends HttpServlet
{
  ...
  private IMethodInfo _renderMethod;
  ...
  private void renderTemplate( ITemplateType templateType, PrintWriter writer, ServletParams params ) {
    if( _renderMethod == null ) {
      _renderMethod = templateType.getTypeInfo().getMethod( "render", TypeSystem.get( Writer.class ), TypeSystem.get(ServletParams.class ) );
    }
    _renderMethod.getCallHandler().handleCall( null, writer, params );
  }
