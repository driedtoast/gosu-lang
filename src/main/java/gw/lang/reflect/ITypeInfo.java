...
public interface ITypeInfo extends IAnnotatedFeatureInfo
{
  ...
  public static class FIND
  {
    ...
    public static IMethodInfo method( List<? extends IMethodInfo> methods, CharSequence method, IType... params )
    {
      params = params == null ? EMPTY_TYPES : params;
      for( IMethodInfo methodInfo : methods )
      {
        if( methodInfo.getDisplayName().equalsIgnoreCase( method.toString() )) {
     ...
     * If you wish strict behavior call {@link #callableMethodStrict(java.util.List, CharSequence, IType[])}
     */
    public static IMethodInfo callableMethod( List<? extends IMethodInfo> methods, CharSequence method, IType... params )
    {
     ...
     */
    public static IMethodInfo callableMethodStrict( List<? extends IMethodInfo> methods, CharSequence method, IType... params )
    {
     ...
     */
    private static IMethodInfo callableMethodImpl( List<? extends IMethodInfo> methods, CharSequence method, boolean strict, IType... params )
    {
      Map<IFunctionType, IMethodInfo> mis = new HashMap<IFunctionType, IMethodInfo>();
      params = params == null ? EMPTY_TYPES : params;
      for( IMethodInfo methodInfo : methods )
      {
        if( methodInfo.getDisplayName().equalsIgnoreCase( method.toString() ) && methodInfo.getParameters().length == params.length )
        {
                                          ...
                                          IType testArgs[] )
    {
      if( srcArgs.length == testArgs.length )
      {
        for( int j = 0; j < srcArgs.length; j++ )
        {
          ...
          if( !methodParamType.equals( testParamType ) )
          {
            return false;
          }
        }
    ...
    private static IType getConcreteBoundingType( IType type )
    {
      ...
      {
        return getConcreteBoundingType( ((ITypeVariableType)type).getTypeVarDef().getTypeVar().getBoundingType() );
      }
